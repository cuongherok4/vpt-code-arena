-- Repair migration for local databases that were baselined before the Exam schema existed.
-- Safe on fresh databases: every DDL statement is guarded with IF NOT EXISTS.
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'judge_result') THEN
...
BEGIN
    IF EXISTS (
        ...
        UPDATE problems SET time_limit_ms = time_limit WHERE time_limit_ms IS NULL;
    END IF;
    ...
    IF EXISTS (
        ...
        UPDATE problems SET memory_limit_kb = memory_limit WHERE memory_limit_kb IS NULL;
    END IF;
END $$;
...
CREATE INDEX IF NOT EXISTS idx_submissions_leaderboard
    ON submissions(problem_id, language, result, submitted_at);
