    ...
    <section className="rounded-lg border border-white/10 bg-slate-950/70 p-4">
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-500">{friends.length} người trong danh sách</p>
        </div>
        <button
          type="button"
          onClick={onRefresh}
      ...
      {loading ? (
        <div className="rounded-lg border border-white/10 bg-white/[0.03] p-4 text-sm text-slate-400">Đang tải danh sách bạn bè...</div>
      ) : friends.length === 0 ? (
        <div className="rounded-lg border border-white/10 bg-white/[0.03] p-4 text-sm text-slate-400">Chưa có bạn bè. Hãy tìm user bằng tên hoặc id để gửi lời mời.</div>
      ) : (
        <div className="space-y-2">
          {friends.map((friend) => (
            <div key={friend.id} className="rounded-lg border border-white/10 bg-white/[0.03] p-3">
              <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                >
                  <div className="relative flex h-10 w-10 items-center justify-center rounded-lg bg-violet-500/10 text-violet-200">
                    <UserRound size={20} />
                    <span className={`absolute -right-0.5 -top-0.5 h-2.5 w-2.5 rounded-full border border-slate-950 ${friend.online ? 'bg-emerald-400' : 'bg-slate-600'}`} />
                  </div>
                  <div className="min-w-0">
                    <p className="truncate text-sm font-semibold text-white">{friend.name}</p>
                    <p className="truncate text-xs text-slate-500">ID {friend.publicId} · {friend.email ?? 'Email ẩn'}</p>
                  </div>
                </button>
